...
namespace Epoint.PingBiao.Common
{
    ...
    public class IOHelper
    {
        ...
        public static void WriteSqlLog(IDbCommand command)
        {
            string log = "SQL=\"" + command.CommandText + "\""+Environment.NewLine;
            foreach (IDataParameter c in command.Parameters)
            {
                log += string.Format("Parameter[\"{0}\"]=\"{1}\"\tDbType={2}", c.ParameterName, c.Value, c.DbType) + Environment.NewLine; 
            }
            Debug.Write(log);
        }
        /// <summary>
        ...
        public static void WriteLog(Exception ex, string appendInfo) {
            string info = "Data:" + ex.Data + Environment.NewLine
            + " InnerException:" + ex.InnerException + Environment.NewLine
            + " Message:" + ex.Message + Environment.NewLine
            + " Source:" + ex.Source + Environment.NewLine
            + " StackTrace:" + ex.StackTrace + Environment.NewLine
            ...
            {
                WriteLog(info + Environment.NewLine
            + " appendInfo:" + appendInfo);
            }
        }

        public static string defaultLogPath = AppDomain.CurrentDomain.BaseDirectory + Path.Combine("App_Data", "Log");
        /// <summary>
        /// 写log
        /// </summary>
        ...
        {
            WriteLog(InfoStr, defaultLogPath);
        }
        /// <summary>
        /// 写log(自动时间log)
        /// </summary>
